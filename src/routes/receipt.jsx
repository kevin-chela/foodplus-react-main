...
export default function Receipt ({ data }){
  ...
  const MapComponent = compose(
    ...
    lifecycle({
      componentDidMount() {
        // Initialize the map and set an initial location
        const map = new window.google.maps.Map(this.props.containerElement, {
          center: { lat: INITIAL_LATITUDE, lng: INITIAL_LONGITUDE },
          zoom: 12,
        });
  
        // Add a click event listener to the map to get the chosen location
        map.addListener('click', (event) => {
          ...
          // Pass the chosen location to a callback function
          this.props.onLocationChosen(chosenLocation);
        });
      },
    })
  )((props) => (
    <GoogleMap
      defaultZoom={8}
      defaultCenter={{ lat: INITIAL_LATITUDE, lng: INITIAL_LONGITUDE }}
    >
      {props.isMarkerShown && (
  ...
  const handleLocationChosen = (location) => {
                    ...
                    style={{
                      ...
                      <MDBCardBody>
                        ...
                        <form className="mt-4">
                         ...
                         <MapComponent
                            isMarkerShown={chosenLocation !== null}
                            markerLat={chosenLocation?.lat}
                            markerLng={chosenLocation?.lng}
                          ...
                          <MDBInput className="mb-4" label="Location" type="text" size="lg"
                            ...
                            value={chosenLocation ? `${chosenLocation.lat}, ${chosenLocation.lng}` : ''}
